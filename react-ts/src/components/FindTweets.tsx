...
const FindTweets: React.FC<FindTweetsProps> = ({ keyword }) => {
  ...
  const fetchPostsByKeyword = useCallback(async () => {
    setLoading(true);
    try {
      const tweetsData = await findPostsByKey(keyword);
      ...
      setError(err.message || "ツイート検索に失敗しました");
    } finally {
      setLoading(false);
    }
  }, [keyword, fetchLikesForPosts]);
  ...
  if (tweets.length === 0) {
    return (
      <Box sx={{ textAlign: "center", mt: 4 }}>
        <Typography variant="h6">検索結果がありません</Typography>
      </Box>
    );
  }
